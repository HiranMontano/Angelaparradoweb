...
_font_css = ""
if _lr:
    _font_css += f"@font-face{{font-family:'Lora';src:url('data:font/truetype;base64,{_lr}') format('truetype');font-weight:400;font-style:normal}}\n"
if _lb:
    _font_css += f"@font-face{{font-family:'Lora';src:url('data:font/truetype;base64,{_lb}') format('truetype');font-weight:700;font-style:normal}}\n"
if _li:
    _font_css += f"@font-face{{font-family:'Lora';src:url('data:font/truetype;base64,{_li}') format('truetype');font-weight:400;font-style:italic}}\n"
if _lbi:
    _font_css += f"@font-face{{font-family:'Lora';src:url('data:font/truetype;base64,{_lbi}') format('truetype');font-weight:700;font-style:italic}}\n"
...
:root{{--orange:#ff6b35;--orange-light:#ff8c61;--orange-dark:#e85a2a;--accent:#ffb347;--black:#0a0a0a;--dark:#1a1a1a;--white:#ffffff;--gray:#9ca3af;--gray-light:#d1d5db}}
html{{scroll-behavior:smooth}}
body{{font-family:'Lora',Georgia,serif;background:var(--black);color:var(--white);overflow-x:hidden;line-height:1.8}}
h1,h2,h3,h4,h5,h6{{font-family:'Goudy Bookletter 1911','Palatino Linotype',Palatino,Georgia,serif;font-weight:normal;letter-spacing:-.5px}}
.section-title,.hero h1,.service-card h3,.about-text h3,.project-card h5,.cta-section h2,.logo{{font-family:'Goudy Bookletter 1911','Palatino Linotype',Palatino,Georgia,serif;font-weight:normal;letter-spacing:-1px}}
...
.btn{{padding:18px 40px;border-radius:12px;font-size:1rem;font-weight:600;text-decoration:none;display:inline-block;transition:all .4s cubic-bezier(.34,1.56,.64,1);position:relative;overflow:hidden;cursor:pointer}}
...
.btn-primary{{background:var(--orange);color:#fff!important;box-shadow:0 10px 40px rgba(255,107,53,.3)}}
.btn-primary:hover{{background:var(--orange-dark);color:#fff!important;transform:translateY(-3px);box-shadow:0 15px 50px rgba(255,107,53,.5)}}
.btn-secondary{{background:var(--orange);color:#fff!important;box-shadow:0 10px 40px rgba(255,107,53,.3)}}
.btn-secondary:hover{{background:var(--orange-dark);color:#fff!important;transform:translateY(-3px);box-shadow:0 15px 50px rgba(255,107,53,.5)}}
.btn-outline{{background:transparent;color:#fff!important;border:2px solid rgba(255,107,53,.5)}}
.btn-outline:hover{{border-color:var(--orange);background:rgba(255,107,53,.1);color:#fff!important;transform:translateY(-3px)}}
...
.logo{{font-size:1.3rem;font-weight:700;letter-spacing:-.5px;color:var(--white)}}
...
.hero h1{{font-size:6.5rem;line-height:1.05;letter-spacing:-3px;margin-bottom:35px;animation:fadeInUp .8s ease .4s backwards}}
...
.btn-download{{display:inline-flex;align-items:center;gap:10px;margin-top:25px;padding:12px 24px;background:transparent;border:1px solid rgba(255,107,53,.5);border-radius:10px;color:var(--orange-light)!important;font-size:.9rem;font-weight:600;text-decoration:none;position:relative;z-index:1;transition:all .3s}}
.btn-download:hover{{background:rgba(255,107,53,.15);border-color:var(--orange);color:var(--white)!important;transform:translateY(-2px)}}
...
.gallery-item::before{{content:'';position:absolute;inset:0;background:linear-gradient(180deg,transparent 30%,rgba(0,0,0,.9) 100%);opacity:0;transition:opacity .4s;z-index:1}}
...
.project-card h5{{font-size:1.5rem;margin-bottom:15px}}
.project-card p{{color:var(--gray);line-height:1.7;font-size:1.05rem;margin-bottom:20px}}
.meta-value{{font-size:1.1rem;font-weight:700;color:var(--orange-light);display:block;margin-bottom:15px}}
.project-meta{{padding-top:20px;border-top:1px solid rgba(255,107,53,.15)}}
...
.cta-section h2{{font-size:5.5rem;margin-bottom:30px;line-height:1.15;letter-spacing:-2px}}
...
.footer-logo{{font-family:'Goudy Bookletter 1911','Palatino Linotype',Georgia,serif;font-size:2rem;margin-bottom:30px;letter-spacing:-1px;display:block}}
footer p{{color:var(--gray);margin-bottom:12px;font-size:.95rem}}

@media(max-width:1200px){{.hero{{grid-template-columns:1fr;gap:60px}}.hero h1{{font-size:5rem!important}}.services-grid{{grid-template-columns:repeat(2,1fr)}}.about-content{{grid-template-columns:1fr;gap:50px}}.about-image{{min-height:350px}}.gallery-grid{{grid-template-columns:repeat(2,1fr);grid-auto-rows:250px}}.gallery-item:nth-child(1),.gallery-item:nth-child(2),.gallery-item:nth-child(3){{grid-column:span 1;grid-row:span 1}}.project-cards{{grid-template-columns:1fr}}}}
@media(max-width:768px){{nav{{padding:25px 30px}}.nav-links{{display:none}}.hero{{padding:120px 30px 60px}}.hero h1{{font-size:3.5rem!important}}.section{{padding:80px 30px}}.about-section{{padding:80px 30px}}.about-text h3{{font-size:2.2rem}}.about-highlights{{grid-template-columns:1fr}}.section-title{{font-size:3rem!important}}.services-grid,.testimonials-grid,.gallery-grid{{grid-template-columns:1fr}}.gallery-grid{{grid-auto-rows:250px}}.cta-section h2{{font-size:3rem}}.hero-buttons,.cta-buttons{{flex-direction:column}}}}
</style>
</head>
<body>
...
<section class="about-section reveal" id="sobre-mi">
  ...
  <div class="about-content">
    ...
    <div class="about-right-col">
      <div class="about-image"><img src="https://im.vsco.co/aws-us-west-2/0a6aa5/136624836/6a0f9681029435f53f3ae4c4/vsco_052126.jpg?w=1600" alt="Sobre mí"></div>
      <div class="about-video-wrapper">
        ...
        <div class="video-container" onclick="loadVideo(this)" data-src="https://www.youtube.com/embed/FZPmelh2-2o?autoplay=1">
          ...
          <button class="play-btn" aria-label="Reproducir video"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></button>
        </div>
      </div>
    </div>
  </div>
...
<section class="gallery-section reveal" id="galeria">
  ...
  <div class="gallery-grid">
    <div class="gallery-item"><img src="https://im.vsco.co/aws-us-west-2/0a6aa5/136624836/6a0fc92772d377d0e5a8c23e/vsco_052126.jpg?w=1600" alt="Evento"><div class="gallery-overlay"><h4>Lanzamiento Expreso Social</h4><p>Producción completa con artistas reconocidos</p></div></div>
    <div class="gallery-item"><img src="https://im.vsco.co/aws-us-west-2/0a6aa5/136624836/6a0fc7f90d87a9ea47de49ca/vsco_052126.jpg?w=1600" alt="Stage Setup"><div class="gallery-overlay"><h4>Stage Setup</h4><p>Montaje técnico profesional</p></div></div>
    <div class="gallery-item"><img src="https://im.vsco.co/aws-us-west-2/0a6aa5/136624836/6a10e0f3acf4897c57d5d875/vsco_052226.jpg?w=1600" alt="Control"><div class="gallery-overlay"><h4>Control Técnico</h4><p>Coordinación en tiempo real</p></div></div>
  </div>
</section>
...
<section class="section experience-section reveal" id="experiencia">
  ...
  <div class="project-cards">
    <div class="project-card">
      ...
      <div class="project-meta"><p>Expreso Social es una promotora de eventos creada junto a otras dos personas, enfocada en desarrollar experiencias más cercanas y conscientes entre artistas emergentes y su público. Dentro del proyecto participo en la planeación y producción de eventos, apoyando procesos logísticos, coordinación general y desarrollo creativo de la experiencia visual y conceptual de cada evento.</p></div>
    </div>
    <div class="project-card">
      <h5>Pabol Producciones - SAS</h5>
      <span class="meta-value">Apoyo Logístico y Producción de Eventos</span>
      <div class="project-meta"><p>He participado en distintos eventos apoyando procesos técnicos, logísticos y operativos. En eventos pequeños he trabajado en montaje y operación básica de audio para presentaciones en vivo, y en producciones de mayor escala he apoyado procesos de logística, coordinación de equipos, montaje y necesidades generales tanto en escenario como en front of house.</p></div>
    </div>
    <div class="project-card">
      <h5>Iglesia del Nazareno Cali</h5>
      <span class="meta-value">Apoyo Stage y Producción en Escenario</span>
      <div class="project-meta"><p>Hago parte de apoyo de stage durante servicios, apoyando procesos como line checks, revisión de instrumentos e input lists, además de facilitar la comunicación entre escenario e ingeniería durante cada culto.</p></div>
    </div>
    <div class="project-card">
      <h5>Universidad Icesi</h5>
      <span class="meta-value">Stage y Backline</span>
      <div class="project-meta"><p>He participado en diferentes eventos musicales e institucionales dentro de la Universidad Icesi, desempeñándome en roles de stage y backline durante presentaciones de ensambles y eventos universitarios. Dentro de estos espacios apoyo la organización de escenario, manejo de tiempos, guía de artistas y logística en vivo.</p></div>
    </div>
  </div>
</section>
...
<section class="section reveal" id="testimonios">
  ...
  <div class="testimonials-grid">
    <div class="testimonial-card"><div class="quote-mark">"</div><p class="testimonial-text">Angela es una persona muy organizada, firmar y clara en sus ideas, además de contar con la capacidad para gestionar de forma rápida y eficiente y es un gran equipo.</p><p class="testimonial-author">Valeria y Veca</p></div>
    <div class="testimonial-card"><div class="quote-mark">"</div><p class="testimonial-text">Comunicación clara y efectiva. Siempre sabe exactamente qué necesitamos en cada momento del evento.</p><p class="testimonial-author">Alexis Play</p></div>
    <div class="testimonial-card"><div class="quote-mark">"</div><p class="testimonial-text">Es una persona con buena atención al cliente y buen manejo de personal. Es eficiente y tranquila bajo presión.</p><p class="testimonial-author">Pabol producciones</p></div>
  </div>
</section>
